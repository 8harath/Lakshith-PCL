"""
Vercel Serverless Function: Crop Recommendation API
Provides crop recommendations based on field conditions
"""
...
def get_recommendations(user_input):
    """Get top crop recommendations based on user inputs"""
    crop_scores = []
    ...
    for crop_name, crop_data in CROP_DATABASE.items():
        ...
        crop_scores.append({
            ...
            'rationale': ' | '.join(result['reasons'][:3])
        })
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        """Handle POST requests for crop recommendations"""
        try:
            # Read request body
            content_length = int(self.headers.get('Content-Length', 0))
            body = self.rfile.read(content_length)
            user_input = json.loads(body.decode('utf-8'))
